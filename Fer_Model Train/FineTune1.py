...
class FER_CNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(128 * 6 * 6, 256),  # increased neurons
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(256, 128),
            nn.ReLU(),
...
# -----------------------------
# Load Pretrained Features Only
# -----------------------------
pretrained_dict = torch.load(PRETRAINED_PATH)
model_dict = model.state_dict()
# Only load feature extractor
pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'features' in k}
model_dict.update(pretrained_dict)
model.load_state_dict(model_dict)
print("Pretrained features loaded, classifier initialized randomly!")

# -----------------------------
# Freeze Feature Extractor (optional)
# -----------------------------
for param in model.features.parameters():
    param.requires_grad = False  # you can later unfreeze some layers for better tuning

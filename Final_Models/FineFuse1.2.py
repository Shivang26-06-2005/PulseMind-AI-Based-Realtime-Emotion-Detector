...
def extract_prosody_features(audio_data, sr=16000):
    ...
    pauses = [ (prev_end, start) for prev_end, (start, end) in zip([0]+[e for s,e in intervals[:-1]], intervals)]
    pause_ratio = float(sum([e-s for s,e in pauses])/duration) if duration>0 else 0.0
    pitch_stress = np.mean(pitch_vals[find_peaks(pitch_vals, prominence=5)[0]]) if len(pitch_vals)>1 else 0.0
    rms_stress = np.mean(rms[find_peaks(rms, prominence=0.01)[0]]) if len(rms)>1 else 0.0
...
# ============================
# 6. Multi-Modal Emotion Detector
# ============================
class MultiModalEmotionDetector:
    def __init__(self):
        ...
        self.sound_model_is_dummy = False
        self.load_models()
    ...
    def load_models(self):
        ...
        try:
            ...
            self.video_model = FER_CNN(7).to(DEVICE)
            self.video_model.eval()
            print("⚠ Video model dummy created")
        # Sound
        try:
            ...
            self.sound_model = TwoLayerCNN(40,92,7).to(DEVICE)
            self.sound_model.eval()
            self.sound_model_is_dummy = True
            print("⚠ Sound model dummy created")
        # Prosody
        try:
            ...
            self.prosody_model.eval()
            print("⚠ Prosody model dummy created")
    ...
    def init_bart_feedback(self):
        try:
            ...
            print("✓ BART feedback loaded")
        except:
            self.bart_tokenizer = None
            self.bart_model = None
            print("⚠ BART feedback failed to load")
    ...
    def analyze_with_bart(self, emotion_text):
        ...
        for aspect in ['engagement','confidence','nervousness','positivity']:
            text = f"The person shows {emotion_text} emotion, indicating {aspect}"
    ...
    def detect_video_emotion_threaded(self, frame):
        if self.processing_video: return
    ...
    def detect_sound_emotion_threaded(self):
        if self.processing_audio: return
    ...
    def analyze_feedback_threaded(self):
        if self.processing_feedback: return
    ...
    def draw_results(self, frame):
        ...
        for i,(x,y,w,h) in enumerate(self.face_locations):
            ...
            cv2.putText(frame,f"Video: {label}",(x,y-10),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,0,0),2)
        panel_height = 200
        panel_width = min(400,width)
        ...
        font_scale = 0.5
        thickness = 1
        cv2.putText(panel,f"Video: {video_emotion}",(10,y_pos),cv2.FONT_HERSHEY_SIMPLEX,font_scale,(0,255,255),thickness)
        y_pos+=line_height
        sound_text = f"{sound_emotion}"
        if self.sound_model_is_dummy:
            sound_text += " ⚠ dummy"
        cv2.putText(panel,f"Sound: {sound_text}",(10,y_pos),cv2.FONT_HERSHEY_SIMPLEX,font_scale,(0,255,255),thickness)
        y_pos+=line_height
        prosody_text = (f"Pitch:{prosody_feats[0]:.1f} Intensity:{prosody_feats[1]:.2f} "
                        f"Tempo:{prosody_feats[2]:.1f} PauseRatio:{prosody_feats[3]:.2f} Stress:{prosody_feats[4]:.2f}")
        cv2.putText(panel,f"Prosody: {prosody_text}",(10,y_pos),cv2.FONT_HERSHEY_SIMPLEX,font_scale,(255,255,0),thickness)
        y_pos+=line_height
        cv2.putText(panel,"Feedback Scores:",(10,y_pos),cv2.FONT_HERSHEY_SIMPLEX,font_scale,(255,255,0),thickness)
        y_pos+=line_height
        for aspect,score in feedback_scores.items():
            color=(0,255,0) if score>0.6 else (0,255,255) if score>0.4 else (0,0,255)
            cv2.putText(panel,f"{aspect[:3]}: {score:.2f}",(10,y_pos),cv2.FONT_HERSHEY_SIMPLEX,font_scale,color,thickness)
            y_pos+=line_height
    ...
    def run(self):
        ...
        while True:
            ...
            if fps_counter>=30:
                ...
                print(f"\n🎬 Video Emotion: {results_copy['video_emotion']}")
                print(f"🔊 Sound Emotion: {results_copy['sound_emotion']}{' ⚠ dummy' if self.sound_model_is_dummy else ''}")
                prosody_labels=['Pitch','Intensity','Tempo','PauseRatio','Stress']
                prosody_values=[f"{val:.2f}" for val in results_copy['prosody_features']]
                prosody_str=", ".join([f"{label}: {val}" for label,val in zip(prosody_labels,prosody_values)])
                print(f"🎵 Prosody Features: {prosody_str}")
                feedback_scores = results_copy['feedback_scores']
                feedback_str=", ".join([f"{k.capitalize()}: {v:.2f}" for k,v in feedback_scores.items()])
                print(f"💡 Feedback Insights: {feedback_str}")
            cv2.imshow('Multi-Modal Emotion Detection',display_frame)
            if cv2.waitKey(1)&0xFF==ord('q'): break
        self.cleanup()
    ...
    def cleanup(self):
        ...
        if hasattr(self,'stream') and self.stream:
            ...
            self.stream.close()
        if hasattr(self,'audio'): self.audio.terminate()

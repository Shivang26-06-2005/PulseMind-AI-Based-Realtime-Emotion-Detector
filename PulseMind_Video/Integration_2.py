...
ROI_SIZE = 50
FPS_SMOOTH = 0.9
N_FRAMES = 8 # Number of frames for the CNN-LSTM model
IMG_SIZE = 112 # Image size for the CNN-LSTM model
...
# Frame history for emotion detection
...
def detect_posture(p_landmarks, w, h):
    ...
    # Check for leaning forward using Z-axis difference
    if z_diff > 0.04:
        return "Leaning Forward", (lw_x, lw_y), (rw_x, rw_y)
    # Check for leaning back using a more sensitive Z-axis threshold
    # Value changed from -0.02 to -0.01 to make it more sensitive
    elif z_diff < -0.01: 
...
# ---------------------------
# CNN-LSTM Model from Pulse_Mind.py
# ---------------------------
...
# ---------------------------
# Emotion Detection with CNN-LSTM
# ---------------------------
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
num_classes = 7
emotion_model = CNN_LSTM(hidden_dim=256, num_classes=num_classes).to(DEVICE)
emotion_model.load_state_dict(torch.load("cnn_lstm_meld_balanced.pth", map_location=DEVICE))
emotion_model.eval()
...
def predict_emotion(frames):
    if len(frames) < N_FRAMES:
        return "Analyzing..."
    
    with torch.no_grad():
        frame_tensor = torch.stack(frames, dim=0).to(DEVICE)
        frame_tensor = frame_tensor.unsqueeze(0)  # Add batch dimension
        outputs = emotion_model(frame_tensor)
...
try:
    while cap.isOpened():
        ...
        frame_history.append(preprocess(frame_rgb))
        ...
        posture = "Neutral"
        lw_px = rw_px = (0, 0)
        emotion_label = predict_emotion(list(frame_history))
